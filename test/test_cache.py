...
class TestRedisCache(unittest.TestCase):
    ...
    def test_time_bucketing(self):
        cache.datetime.datetime.utcnow.return_value = datetime.datetime(2019, 11, 11, 9, 0, 0)
        _ = self.c('u1', '1', True)
        r = self.c('u1')
        self.assertCountEqual(r, ['1'])

        cache.datetime.datetime.utcnow.return_value = datetime.datetime(2019, 11, 11, 18, 0, 0)
        r = self.c('u1')
        self.assertCountEqual(r, ['1'])

        cache.datetime.datetime.utcnow.return_value = datetime.datetime(2019, 11, 12, 9, 0, 0)
        r = self.c('u1')
        self.assertCountEqual(r, ['1'])

        cache.datetime.datetime.utcnow.return_value = datetime.datetime(2019, 11, 13, 9, 0, 0)
        r = self.c('u1')
        self.assertCountEqual(r, [])
